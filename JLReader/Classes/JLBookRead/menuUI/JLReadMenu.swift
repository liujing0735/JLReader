...
@objc protocol JLReadMenuDelegate:NSObjectProtocol {
    
    /// 状态栏 将要 - 隐藏以及显示状态改变
    @objc optional func readMenuWillShowOrHidden(readMenu:JLReadMenu, isShow:Bool)
    
    /// 状态栏 完成 - 隐藏以及显示状态改变
    @objc optional func readMenuDidShowOrHidden(readMenu:JLReadMenu, isShow:Bool)
    
    /// 点击下载
    @objc optional func readMenuClickDownload(readMenu:JLReadMenu)
    
    /// 点击书签按钮
    @objc optional func readMenuClickMarkButton(readMenu:JLReadMenu, button:UIButton)
    
    /// 点击上一章（上一话）
    @objc optional func readMenuClickPreviousChapter(readMenu:JLReadMenu)
    
    /// 点击下一章（下一话）
    @objc optional func readMenuClickNextChapter(readMenu:JLReadMenu)
    
    /// 停止滚动进度条
    @objc optional func readMenuSliderEndScroll(readMenu:JLReadMenu,slider:ASValueTrackingSlider)
    
    /// 点击背景颜色
    @objc optional func readMenuClickSetuptColor(readMenu:JLReadMenu,index:NSInteger,color:UIColor)
    
    /// 点击翻书动画
    @objc optional func readMenuClickSetuptEffect(readMenu:JLReadMenu,index:NSInteger)
    
    /// 点击字体
    @objc optional func readMenuClickSetuptFont(readMenu:JLReadMenu,index:NSInteger)
    
    /// 点击字体大小
    @objc optional func readMenuClickSetuptFontSize(readMenu:JLReadMenu,fontSize:CGFloat)
    
    /// 点击日间夜间
    @objc optional func readMenuClickLightButton(readMenu:JLReadMenu,isDay:Bool)
    
    /// 点击章节列表
    @objc optional func readMenuClickChapterList(readMenu:JLReadMenu,readChapterListModel:JLReadChapterListModel)
    
    /// 点击书签列表
    @objc optional func readMenuClickMarkList(readMenu:JLReadMenu,readMarkModel:JLReadMarkModel)
    
}

class JLReadMenu: NSObject,UIGestureRecognizerDelegate {
    
    /// 控制器
    private(set) weak var vc:JLReadController!
    
    /// 代理
    private(set) weak var delegate:JLReadMenuDelegate!
    ...
    /// LeftView
    private(set) var leftView:JLRMLeftView!
    
    /// TopView
    private(set) var topView:JLRMTopView!
    
    /// BottomView
    private(set) var bottomView:JLRMBottomView!
    
    /// 亮度
    private(set) var lightView:JLRMLightView!
    
    /// 遮盖亮度
    private var coverView:UIView!
    
    /// 亮度按钮
    private var lightButton:JLHaloButton!
    ...
    class func readMenu(vc:JLReadController,delegate:JLReadMenuDelegate) ->JLReadMenu {
        ...
        let readMenu = JLReadMenu(vc:vc,delegate:delegate)
        
        return readMenu
    }
    
    /// 初始化函数
    private init(vc:JLReadController,delegate:JLReadMenuDelegate) {
    ...
    private func creatUI() {
        ...
        // 设置为日夜间 默认日间
        lightButton.isSelected = JLUserDefaults.boolForKey(JLKey_IsNighOrtDay)
    }
    ...
    /// 点击这些控件不需要执行手势
    private let ClassString:[String] = ["ASValueTrackingSlider","UITableViewCellContentView","UISlider","JLRMLeftView","JLRMTopView","JLRMBottomView","JLHaloButton","JLRMLightView","JLRMColorView","JLRMFuncView","JLSettingView","UIButton"]
    ...
    private func initLeftView() {
        ...
        leftView.addTarget(self, action: #selector(JLReadMenu.clickLeftView), for: .touchUpInside)
    ...
    private func initCoverView() {
        ...
        coverView.alpha = JLUserDefaults.boolForKey(JLKey_IsNighOrtDay) ? 1.0 : 0
    ...
    private func initLightView() {
        ...
        lightView = JLRMLightView(readMenu:self)
    ...
    private func initLightButton() {
        ...
        lightButton.addTarget(self, action: #selector(JLReadMenu.clickLightButton), for: .touchUpInside)
    }
    ...
    @objc func clickLightButton(button:UIButton) {
        ...
        JLUserDefaults.setBool(button.isSelected, key: JLKey_IsNighOrtDay)
    ...
    private func initTopView() {
        ...
        topView = JLRMTopView(readMenu:self)
    ...
    private func initBottomView() {
        ...
        bottomView = JLRMBottomView(readMenu:self)
