...
/// 小说FuncView支持类型
enum JLRMFuncViewType:NSInteger {
    case effect
    case font
    case fontSize
}
...
class JLRMFuncView: JLRMBaseView {
    ...
    /// 类型
    private(set) var funcType:JLRMFuncViewType!
    ...
    /// 初始化方法
    convenience init(frame:CGRect,readMenu:JLReadMenu,funcType:JLRMFuncViewType,title:String) {
        
        self.init(frame:frame,readMenu:readMenu,funcType:funcType,title:title,labels:[],selectIndex:0)
    }
    
    /// 初始化方法
    init(frame:CGRect,readMenu:JLReadMenu,funcType:JLRMFuncViewType,title:String,labels:[String],selectIndex:NSInteger) {
    ...
    override func addSubviews() {
        ...
        titleLabel.frame = CGRect(x: JLSpace_2, y: 0, width: 55, height: height)
        ...
        if funcType == .font || funcType == .effect {
            ...
            for i in 0..<count {
                ...
                button.titleLabel?.font = JLFont_12
                ...
                button.setTitleColor(JLColor_2, for: .selected)
            ...
            let buttonW:CGFloat = (contentW - JLSpace_1 + JLSpace_3) / 2
            let buttonSpaceW:CGFloat = JLSpace_3
    ...
    @objc func clickFontSize(button:UIButton) {
        ...
        if button.tag == 0 { // left
            ...
            // 没有小于最小字体
            if (JLReadConfigure.shared().fontSize - fontSpace) >= JLReadMinFontSize {
                
                JLReadConfigure.shared().fontSize -= fontSpace
                
                readMenu.delegate?.readMenuClickSetuptFontSize?(readMenu: readMenu, fontSize: CGFloat(JLReadConfigure.shared().fontSize))
            }
            
        }else{ // right
            
            // 没有大于最大字体
            if (JLReadConfigure.shared().fontSize + fontSpace) <= JLReadMaxFontSize {
