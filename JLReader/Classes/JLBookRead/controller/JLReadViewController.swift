...
class JLReadViewController: UIViewController,UITableViewDelegate,UITableViewDataSource {
    
    /// 临时阅读记录
    var readRecordModel:JLReadRecordModel!
    
    /// 阅读控制器
    weak var readController:JLReadController!
    
    /// 顶部状态栏
    private(set) var topStatusView:UILabel!
    
    /// 底部状态栏
    private(set) var bottomStatusView:JLRMStatusView!
    ...
    /// 当前阅读View(上下滚动不能使用)
    private weak var readView:JLReadView?
    ...
    func addSubviews() {
        ...
        // BottomStatusView
        bottomStatusView = JLRMStatusView(frame:CGRect(x: JLSpace_1, y: view.frame.height - JLSpace_2, width: view.width - 2 * JLSpace_1, height: JLSpace_2))
        ...
        // 设置数据源
        if JLReadConfigure.shared().effectType == JLRMEffectType.upAndDown.rawValue && readRecordModel.readChapterModel != nil {
    ...
    func configureBGColor() {
        ...
        view.backgroundColor = JLReadConfigure.shared().readColor()
    }
    ...
    func configureReadEffect() {
        ...
        if JLReadConfigure.shared().effectType != JLRMEffectType.upAndDown.rawValue { // 非上下滚动
    ...
    func configureReadRecordModel() {
        ...
        if JLReadConfigure.shared().effectType == JLRMEffectType.upAndDown.rawValue { // 上下滚动
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        ...
        if JLReadConfigure.shared().effectType != JLRMEffectType.upAndDown.rawValue { // 非上下滚动
            ...
            let readChapterModel = JLReadChapterModel.readChapterModel(bookID: readRecordModel.bookID, chapterID: dataArray[section], isUpdateFont: true)
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if JLReadConfigure.shared().effectType != JLRMEffectType.upAndDown.rawValue { // 非上下滚动
            ...
            let cell = JLReadViewCell.cellWithTableView(tableView)
            ...
            let readChapterModel = JLReadChapterModel.readChapterModel(bookID: readRecordModel.bookID, chapterID: dataArray[indexPath.section], isUpdateFont: true)
    ...
    /// 动态添加数据源
    func reloadDataArray(readChapterModel:JLReadChapterModel) {
        ...
        if lastChapterId != nil && !willLoadDataArray.contains(lastChapterId!) {
            ...
            DispatchQueue.global().async { [weak self] () in
                ...
                if !self!.dataArray.contains(lastChapterId!) {
                    ...
                    // 获取上一章模型
                    let readChapterModel = JLReadChapterModel.readChapterModel(bookID: readChapterModel.bookID, chapterID: lastChapterId!, isUpdateFont: true)
        ...
        if nextChapterId != nil && !willLoadDataArray.contains(nextChapterId!) {
            ...
            DispatchQueue.global().async { [weak self] () in
                ...
                if !self!.dataArray.contains(nextChapterId!) {
                    ...
                    // 获取上一章模型
                    let readChapterModel = JLReadChapterModel.readChapterModel(bookID: readChapterModel.bookID, chapterID: nextChapterId!, isUpdateFont: true)
    ...
    func updateReadRecordModel() {
        ...
        if (JLReadConfigure.shared().effectType == JLRMEffectType.upAndDown.rawValue) { // 上下滚动
            ...
            if tableView.indexPathsForVisibleRows != nil && !tableView.indexPathsForVisibleRows!.isEmpty { // 有Cell
                ...
                // 显示章节Cell IndexPath
                var indexPath = tableView.indexPathsForRows(in: CGRect(x: 0, y: tableView.contentOffset.y + JLSpace_1, width: rect.width, height: rect.height - JLSpace_1))!.first!
