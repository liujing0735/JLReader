...
class JLReadParser: NSObject {
    ...
    /**
     ...
     - returns: ReadModel
     */
    @objc class func ParserLocalURL(url:URL) ->JLReadModel {
        ...
        if !JLReadModel.IsExistReadModel(bookID: bookID) { // 不存在
            ...
            // 解析数据
            let content = JLReadParser.EncodeURL(url)
            ...
            // 返回
            return JLReadModel.readModel(bookID: bookID)
        }
    }
    ...
    /**
     ...
     - returns: 章节列表模型数组
     */
    private class func ParserContent(bookID:String, content:String) ->[JLReadChapterListModel] {
        ...
        if !results.isEmpty {
            ...
            // 记录 上一章 模型
            var lastReadChapterModel:JLReadChapterModel?
            ...
            for i in 0...count {
                ...
                // 创建章节内容模型
                let readChapterModel = JLReadChapterModel()
    ...
    /**
     ...
     - returns: 阅读章节列表模型
     */
    private class func GetReadChapterListModel(readChapterModel:JLReadChapterModel) ->JLReadChapterListModel {
        
        let readChapterListModel = JLReadChapterListModel()
