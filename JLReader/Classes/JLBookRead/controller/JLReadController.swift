...
class JLReadController: JLViewController,JLReadMenuDelegate,JLCoverControllerDelegate,UIPageViewControllerDelegate,UIPageViewControllerDataSource {
    
    /// 阅读模型(必传)
    @objc var readModel:JLReadModel!
    
    /// 开启长按菜单
    @objc var openLongMenu:Bool = true
    
    /// 阅读菜单UI
    private(set) var readMenu:JLReadMenu!
    
    /// 阅读操作对象
    private(set) var readOperation:JLReadOperation!
    
    /// 翻页控制器 (仿真)
    private(set) var pageViewController:UIPageViewController?
    
    /// 翻页控制器 (无效果,覆盖,上下)
    private(set) var coverController:JLCoverController?
    
    /// 当前显示的阅读控制器
    private(set) var currentReadViewController:JLReadViewController?
    ...
    override func viewDidLoad() {
        ...
        // 初始化控制器操作对象
        readOperation = JLReadOperation(vc: self)
        
        // 初始化阅读UI控制对象
        readMenu = JLReadMenu.readMenu(vc: self, delegate: self)
        
        // 初始化控制器
        creatPageController(readOperation.GetCurrentReadViewController(isUpdateFont: true, isSave: true))
        
        // 注册 JLReadView 手势通知
        JLReadView.RegisterNotification(observer: self, selector: #selector(readViewNotification(notification:)))
    }
    
    // MARK: JLReadView 手势通知
    ...
    @objc func readViewNotification(notification:Notification) {
        ...
        // 解析状态
        if info != nil && info!.keys.contains(JLKey_ReadView_Ges_isOpen) {
            
            let isOpen = info![JLKey_ReadView_Ges_isOpen] as! NSNumber
    ...
    // MARK: -- JLReadMenuDelegate
    
    /// 背景颜色
    func readMenuClickSetuptColor(readMenu: JLReadMenu, index: NSInteger, color: UIColor) {
        
        JLReadConfigure.shared().colorIndex = index
        
        currentReadViewController?.configureBGColor()
    }
    
    /// 翻书动画
    func readMenuClickSetuptEffect(readMenu: JLReadMenu, index: NSInteger) {
        
        JLReadConfigure.shared().effectType = index
        
        creatPageController(readOperation.GetCurrentReadViewController())
    }
    
    /// 字体
    func readMenuClickSetuptFont(readMenu: JLReadMenu, index: NSInteger) {
        
        JLReadConfigure.shared().fontType = index
        
        creatPageController(readOperation.GetCurrentReadViewController(isUpdateFont: true, isSave: true))
    }
    
    /// 字体大小
    func readMenuClickSetuptFontSize(readMenu: JLReadMenu, fontSize: CGFloat) {
        
        // JLReadConfigure.shared().fontSize = fontSize 内部已赋值
        
        creatPageController(readOperation.GetCurrentReadViewController(isUpdateFont: true, isSave: true))
    }
    
    /// 点击书签列表
    func readMenuClickMarkList(readMenu: JLReadMenu, readMarkModel: JLReadMarkModel) {
    ...
    /// 下载
    func readMenuClickDownload(readMenu: JLReadMenu) {
    ...
    /// 下一章
    func readMenuClickNextChapter(readMenu: JLReadMenu) {
    ...
    /// 点击章节列表
    func readMenuClickChapterList(readMenu: JLReadMenu, readChapterListModel: JLReadChapterListModel) {
        
        let _ = readOperation.GoToChapter(chapterID: readChapterListModel.id)
    }
    
    /// 切换日夜间模式
    func readMenuClickLightButton(readMenu: JLReadMenu, isDay: Bool) {
    ...
    /// 状态栏 将要 - 隐藏以及显示状态改变
    func readMenuWillShowOrHidden(readMenu: JLReadMenu, isShow: Bool) {
    ...
    /// 点击书签按钮
    func readMenuClickMarkButton(readMenu: JLReadMenu, button: UIButton) {
    ...
    func creatPageController(_ displayController:UIViewController?) {
        ...
        if JLReadConfigure.shared().effectType == JLRMEffectType.simulation.rawValue { // 仿真
            ...
            coverController = JLCoverController()
            ...
            if JLReadConfigure.shared().effectType == JLRMEffectType.none.rawValue {
                
                coverController!.openAnimate = false
                
            }else if JLReadConfigure.shared().effectType == JLRMEffectType.upAndDown.rawValue {
        ...
        // 记录
        currentReadViewController = displayController as? JLReadViewController
    }
    ...
    // MARK: -- JLCoverControllerDelegate
    
    /// 切换结果
    func coverController(_ coverController: JLCoverController, currentController: UIViewController?, finish isFinish: Bool) {
        
        // 记录
        currentReadViewController = currentController as? JLReadViewController
    ...
    /// 将要显示的控制器
    func coverController(_ coverController: JLCoverController, willTransitionToPendingController pendingController: UIViewController?) {
        
        readMenu.menuSH(isShow: false)
    }
    
    /// 获取上一个控制器
    func coverController(_ coverController: JLCoverController, getAboveControllerWithCurrentController currentController: UIViewController?) -> UIViewController? {
        
        return readOperation.GetAboveReadViewController()
    }
    
    /// 获取下一个控制器
    func coverController(_ coverController: JLCoverController, getBelowControllerWithCurrentController currentController: UIViewController?) -> UIViewController? {
    ...
    func pageViewController(_ pageViewController: UIPageViewController, didFinishAnimating finished: Bool, previousViewControllers: [UIViewController], transitionCompleted completed: Bool) {
        ...
        if !completed {
            ...
            // 记录
            currentReadViewController = previousViewControllers.first as? JLReadViewController
            ...
            // 记录
            currentReadViewController = pageViewController.viewControllers?.first as? JLReadViewController
    ...
    func pageViewController(_ pageViewController: UIPageViewController, willTransitionTo pendingViewControllers: [UIViewController]) {
        ...
        // 更新阅读记录
        readOperation.readRecordUpdate(readViewController: pageViewController.viewControllers?.first as? JLReadViewController, isSave: false)
    }
    ...
    deinit {
        ...
        // 移除通知
        JLReadView.RemoveNotification(observer: self)
