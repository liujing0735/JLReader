...
class JLReadOperation: NSObject {

    /// 阅读控制器
    weak var vc:JLReadController!
    
    // MARK: -- init
    
    init(vc:JLReadController) {
    ...
    // MARK: -- 获取阅读控制器 JLReadViewController
    
    /// 获取阅读View控制器
    func GetReadViewController(readRecordModel:JLReadRecordModel?) ->JLReadViewController? {
        
        if readRecordModel != nil {
            
            let readViewController = JLReadViewController()
    ...
    /// 获取当前阅读记录的阅读View控制器
    func GetCurrentReadViewController(isUpdateFont:Bool = false, isSave:Bool = false) ->JLReadViewController? {
    ...
    /// 获取上一页控制器
    func GetAboveReadViewController() ->JLReadViewController? {
        ...
        if vc.readModel.isLocalBook.boolValue { // 本地小说
            ...
            if page == 0 { // 这一章到头了
                ...
                readRecordModel?.modify(chapterID: "\(id - 1)", toPage: JLReadLastPageValue, isUpdateFont:true, isSave: false)
    ...
    /// 获得下一页控制器
    func GetBelowReadViewController() ->JLReadViewController? {
    ...
    /// 跳转指定章节 指定页码 (toPage: -1 为最后一页 也可以使用 JLReadLastPageValue)
    func GoToChapter(chapterID:String, toPage:NSInteger = 0) ->Bool {
        
        if vc.readModel != nil { // 有阅读模型
            
            if JLReadChapterModel.IsExistReadChapterModel(bookID: vc.readModel.bookID, chapterID: chapterID) { //  存在
    ...
    /// 更新记录
    func readRecordUpdate(readViewController:JLReadViewController?, isSave:Bool = true) {
       
        readRecordUpdate(readRecordModel: readViewController?.readRecordModel, isSave: isSave)
    }
    
    /// 更新记录
    func readRecordUpdate(readRecordModel:JLReadRecordModel?, isSave:Bool = true) {
