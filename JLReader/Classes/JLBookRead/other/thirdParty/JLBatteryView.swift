...
/// 电池宽推荐使用宽高
var JLBatterySize:CGSize = CGSize(width: 25, height: 10)

/// 电池量宽度 跟图片的比例
private var HJBatteryLevelViewW:CGFloat = 20
private var HJBatteryLevelViewScale = HJBatteryLevelViewW / JLBatterySize.width

import UIKit

class JLBatteryView: UIImageView {
    ...
    convenience init() {
        ...
        self.init(frame: CGRect(x: 0, y: 0, width: JLBatterySize.width, height: JLBatterySize.height))
    }
    
    /// 初始化
    override init(frame: CGRect) {
        
        super.init(frame: CGRect(x: 0, y: 0, width: JLBatterySize.width, height: JLBatterySize.height))
    ...
    override func layoutSubviews() {
        ...
        let spaceW:CGFloat = 1 * (frame.width / JLBatterySize.width) * HJBatteryLevelViewScale
        let spaceH:CGFloat = 1 * (frame.height / JLBatterySize.height) * HJBatteryLevelViewScale
