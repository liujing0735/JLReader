...
class JLReadChapterModel: NSObject,NSCoding {
    ...
    func updateFont(isSave:Bool = false) {
        ...
        let readAttribute = JLReadConfigure.shared().readAttribute(isPaging: true)
        
        if !NSDictionary(dictionary: self.readAttribute).isEqual(to: readAttribute) {
            
            self.readAttribute = readAttribute
            
            rangeArray = JLReadParser.ParserPageRange(string: content, rect: GetReadViewFrame(), attrs: readAttribute)
    ...
    class func readChapterModel(bookID:String, chapterID:String, isUpdateFont:Bool = false) ->JLReadChapterModel {
        ...
        if JLReadChapterModel.IsExistReadChapterModel(bookID: bookID, chapterID: chapterID) { // 存在
            ...
            readChapterModel = JLReadChapterModel()
