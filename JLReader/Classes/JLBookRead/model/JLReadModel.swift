...
class JLReadModel: NSObject,NSCoding {
    ...
    var readChapterListModels:[JLReadChapterListModel] = [JLReadChapterListModel]()
    
    /// 阅读记录
    var readRecordModel:JLReadRecordModel!
    
    /// 书签列表
    private(set) var readMarkModels:[JLReadMarkModel] = [JLReadMarkModel]()
    
    /// 当前书签(用于记录使用)
    private(set) var readMarkModel:JLReadMarkModel?
    ...
    /// 获得阅读模型
    class func readModel(bookID:String) ->JLReadModel {
        
        var readModel:JLReadModel!
        ...
        if JLReadModel.IsExistReadModel(bookID: bookID) { // 存在
            ...
            readModel = JLReadModel()
            
            readModel.bookID = bookID
        }
        
        // 阅读记录 刷新字体是以防在别的小说修改了字体
        readModel!.readRecordModel = JLReadRecordModel.readRecordModel(bookID: bookID, isUpdateFont: true, isSave: true)
    ...
    /// 修改阅读记录到书签模型
    func modifyReadRecordModel(readMarkModel:JLReadMarkModel, isUpdateFont:Bool = false, isSave:Bool = false) {
    ...
    /// 通过ID获得章节列表模型
    func GetReadChapterListModel(chapterID:String) ->JLReadChapterListModel? {
    ...
    /// 添加书签 默认使用当前阅读记录作为书签
    func addMark(readRecordModel:JLReadRecordModel? = nil) {
        
        let readRecordModel = (readRecordModel != nil ? readRecordModel : self.readRecordModel)!
        
        let readMarkModel = JLReadMarkModel()
    ...
    /// 删除书签 默认使用当前存在的书签
    func removeMark(readMarkModel:JLReadMarkModel? = nil, index:NSInteger? = nil) ->Bool {
    ...
    func checkMark(readRecordModel:JLReadRecordModel? = nil) ->Bool {
        
        let readRecordModel = (readRecordModel != nil ? readRecordModel : self.readRecordModel)!
        
        let chapterID = readRecordModel.readChapterModel!.id
        
        var results:[JLReadMarkModel] = []
    ...
    required init?(coder aDecoder: NSCoder) {
        ...
        readChapterListModels = aDecoder.decodeObject(forKey: "readChapterListModels") as! [JLReadChapterListModel]
        
        readMarkModels = aDecoder.decodeObject(forKey: "readMarkModels") as! [JLReadMarkModel]
    }
