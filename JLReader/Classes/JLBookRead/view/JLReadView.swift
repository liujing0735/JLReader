...
class JLReadView: UIView {
    ...
    var content:String? {
        ...
        didSet{
            ...
            if content != nil && !content!.isEmpty {
                ...
                frameRef = JLReadParser.GetReadFrameRef(content: content!, attrs: JLReadConfigure.shared().readAttribute(), rect: GetReadViewFrame())
            }
        }
    }
    ...
    /// 左光标
    private var LCursorView:JLCursorView!
    
    /// 右光标
    private var RCursorView:JLCursorView!
    ...
    /// 放大镜
    private var magnifierView:JLMagnifierView?
    ...
    @objc private func longAction(long:UILongPressGestureRecognizer) {
        ...
        // 禁止上下滚动使用
        if JLReadConfigure.shared().effectType == JLRMEffectType.upAndDown.rawValue { return }
        ...
        if long.state == .began {
            ...
            // 发送通知
            JLReadView.PostNotification(userInfo: [JLKey_ReadView_Ges_isOpen: NSNumber(value: true)])
            ...
            // 获得选中区域
            selectRange = JLReadAuxiliary.GetTouchLineRange(point: point, frameRef: frameRef)
            
            // 获得选中选中范围
            rects = JLReadAuxiliary.GetRangeRects(range: selectRange!, frameRef: frameRef, content: content)
            ...
            if !rects.isEmpty {
                ...
                // 发送通知
                JLReadView.PostNotification(userInfo: [JLKey_ReadView_Ges_isOpen: NSNumber(value: false)])
            }
        }
    }
    ...
    /// 解析触摸事件
    private func drag(touches: Set<UITouch>, status: JLPanStatus) {
    ...
    /// 拖拽事件解析
    func drag(status:JLPanStatus, point:CGPoint, windowPoint:CGPoint) {
        ...
        if status == .begin {
            ...
            if LCursorView.frame.insetBy(dx: JLCursorOffset, dy: JLCursorOffset).contains(point) { // 触摸到左边光标
                
                isLorR_Cursor = true
                
                isTouchCursor = true
                
            }else if RCursorView.frame.insetBy(dx: JLCursorOffset, dy: JLCursorOffset).contains(point) { // 触摸到右边光标
            ...
            if isTouchCursor && selectRange != nil {
                ...
                // 触摸到的位置
                let location = JLReadAuxiliary.GetTouchLocation(point: point, frameRef: frameRef)
    ...
    private func cursor(isShow:Bool) {
        ...
        if isShow {
            ...
            if !rects.isEmpty && LCursorView == nil {
                ...
                LCursorView = JLCursorView()
                LCursorView.isTorB = true
                addSubview(LCursorView)
                
                RCursorView = JLCursorView()
    ...
    override func draw(_ rect: CGRect) {
        ...
        if selectRange != nil && !rects.isEmpty {
            ...
            JLColor_2.withAlphaComponent(0.5).setFill()
    ...
    private func menu(isShow:Bool) {
        ...
        if isShow { // 显示
            ...
            if !rects.isEmpty {
                ...
                let rect = JLReadAuxiliary.GetMenuRect(rects: rects, viewFrame: bounds)
    ...
    class func RegisterNotification(observer:Any, selector:Selector) {
        ...
        NotificationCenter.default.addObserver(observer, selector: selector, name: NSNotification.Name(rawValue: JLNotificationName_ReadView_Ges), object: nil)
    }
    
    /// 移除通知
    class func RemoveNotification(observer:Any) {
        
        NotificationCenter.default.removeObserver(observer, name: NSNotification.Name(rawValue: JLNotificationName_ReadView_Ges), object: nil)
    }
    
    /// 发送通知
    class func PostNotification(userInfo:[AnyHashable : Any]?) {
        
        NotificationCenter.default.post(name: NSNotification.Name(rawValue: JLNotificationName_ReadView_Ges), object: nil, userInfo: userInfo)
    }
